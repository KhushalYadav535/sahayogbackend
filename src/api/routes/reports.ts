...
router.get("/custom", authMiddleware, requireTenant, async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        const tenantId = req.user!.tenantId!;
        const { module, limit = "500" } = req.query as Record<string, string>;
...
router.get("/npa-trend", authMiddleware, requireTenant, async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        const tenantId = req.user!.tenantId!;
        const today = new Date();
        ...
        const loansWithDpd = await Promise.all(
        ...
        for (const l of loansWithDpd) {
            ...
            if (l.dpd === 0) dpdBuckets[0].count++, (dpdBuckets[0].outstanding += out);
            else if (l.dpd <= 29) dpdBuckets[1].count++, (dpdBuckets[1].outstanding += out);
            else if (l.dpd <= 59) dpdBuckets[2].count++, (dpdBuckets[2].outstanding += out);
            else if (l.dpd <= 89) dpdBuckets[3].count++, (dpdBuckets[3].outstanding += out);
            else if (l.dpd <= 364) dpdBuckets[4].count++, (dpdBuckets[4].outstanding += out);
            else if (l.dpd <= 1095) dpdBuckets[5].count++, (dpdBuckets[5].outstanding += out);
            else dpdBuckets[6].count++, (dpdBuckets[6].outstanding += out);
        }
        ...
        const monthlyNPA: { month: string; total: number; sub: number; doubtful: number; loss: number }[] = [];
        ...
        if (monthlyNPA.length === 0) monthlyNPA.push({ month: months[today.getMonth()], total: npaRatio, sub: npaRatio * 0.55, doubtful: npaRatio * 0.3, loss: npaRatio * 0.15 });
...
router.get("/risk", authMiddleware, requireTenant, async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        const tenantId = req.user!.tenantId!;
        const today = new Date();
        ...
        const loansWithDpd = await Promise.all(
            allLoans.map(async (l) => ({ ...l, dpd: await getLoanDpd(l.id) }))
        );
        ...
        const highRisk = npaLoans
            ...
            .map((l) => {
                const flags: string[] = [];
                if (l.dpd >= 90) flags.push("NPA");
                if (Number(l.outstandingPrincipal) > totalPortfolio / (allLoans.length || 1) * 2) flags.push("High Exposure");
...
router.get("/portfolio", authMiddleware, requireTenant, async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        const tenantId = req.user!.tenantId!;
        const today = new Date();
        ...
        const byType: Record<string, { size: number; count: number; npa: number; npaCount: number }> = {};
        for (const l of loans) {
            const type = l.loanType;
            if (!byType[type]) byType[type] = { size: 0, count: 0, npa: 0, npaCount: 0 };
            byType[type].size += Number(l.outstandingPrincipal);
            byType[type].count++;
            if (l.status === "npa") byType[type].npaCount++;
        }
...
router.get("/director-kpi", authMiddleware, requireTenant, async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        const tenantId = req.user!.tenantId!;
        const today = new Date();
        ...
        if (today.getMonth() < 2) fyStart.setFullYear(fyStart.getFullYear() - 1);
...
router.get("/member-analytics", authMiddleware, requireTenant, async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        const tenantId = req.user!.tenantId!;
        const today = new Date();
