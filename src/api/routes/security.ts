...
import { authMiddleware, requireRole, AuthRequest, requireTenant } from "../middleware/auth";
...
router.get("/permissions", authMiddleware, requireTenant, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const tenantId = req.user!.tenantId!;
    const matrices = await prisma.permissionMatrix.findMany({
...
router.post("/permissions", authMiddleware, requireTenant, requireRole("superadmin", "admin"), async (req: AuthRequest, res: Response): Promise<void> => {
  try {
...
router.post("/roles/assign", authMiddleware, requireTenant, requireRole("superadmin", "admin"), async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { userId, role, reason } = z.object({
      userId: z.string(),
      role: z.string(),
...
router.get("/roles/assignments", authMiddleware, requireTenant, requireRole("superadmin", "admin"), async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const tenantId = req.user!.tenantId!;
    const assignments = await prisma.roleAssignmentLog.findMany({
      where: { tenantId },
...
router.post("/mfa/setup", authMiddleware, requireTenant, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
...
router.post("/mfa/verify", authMiddleware, requireTenant, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { code, backupCode } = z.object({
    ...
    if (backupCode && user.totpBackupCodes.includes(backupCode)) {
      ...
      const updatedBackupCodes = user.totpBackupCodes.filter((c: string) => c !== backupCode);
      await prisma.user.update({
        where: { id: userId },
...
router.post("/mfa/disable", authMiddleware, requireTenant, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const userId = req.user!.userId!;
    const user = await prisma.user.findUnique({ where: { id: userId } });
    if (!user) {
...
router.get("/mfa/status", authMiddleware, requireTenant, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const userId = req.user!.userId!;
...
router.post("/dpdp/access-request", authMiddleware, requireTenant, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
...
router.get("/dpdp/access-requests", authMiddleware, requireTenant, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const tenantId = req.user!.tenantId!;
...
router.post("/dpdp/access-requests/:id/fulfill", authMiddleware, requireTenant, requireRole("admin", "secretary"), async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    const tenantId = req.user!.tenantId!;

    const request = await prisma.dataAccessRequest.findUnique({ where: { id } });
...
router.post("/dpdp/correction-request", authMiddleware, requireTenant, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { memberId, field, newValue, reason } = z.object({
      memberId: z.string(),
      field: z.string(),
      newValue: z.string(),
      reason: z.string().optional(),
    ...
    const oldValue = (member as any)[field] || null;
...
router.post("/dpdp/correction-requests/:id/approve", authMiddleware, requireTenant, requireRole("admin", "secretary"), async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    const tenantId = req.user!.tenantId!;

    const request = await prisma.dataCorrectionRequest.findUnique({ where: { id } });
    if (!request || request.tenantId !== tenantId) {
      res.status(404).json({ success: false, message: "Request not found" });
      return;
    }
    ...
    // Update member field
    const updateData: any = { [request.field]: request.newValue };
    ...
    // Update request
    const updated = await prisma.dataCorrectionRequest.update({
      where: { id },
      data: {
        status: "APPROVED",
...
router.post("/dpdp/erasure-request", authMiddleware, requireTenant, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { memberId, erasureType, reason } = z.object({
      memberId: z.string(),
      erasureType: z.enum(["FULL", "PARTIAL", "ANONYMIZE"]),
      reason: z.string().optional(),
    }).parse(req.body);
...
router.post("/dpdp/erasure-requests/:id/process", authMiddleware, requireTenant, requireRole("admin"), async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    const tenantId = req.user!.tenantId!;

    const request = await prisma.dataErasureRequest.findUnique({ where: { id } });
    if (!request || request.tenantId !== tenantId) {
      res.status(404).json({ success: false, message: "Request not found" });
      return;
    }

    const member = await prisma.member.findUnique({ where: { id: request.memberId } });
    if (!member) {
      res.status(404).json({ success: false, message: "Member not found" });
      return;
    }
    ...
    const retentionFields: string[] = ["id", "tenantId", "memberNumber"]; // Legal compliance fields
    ...
    const updated = await prisma.dataErasureRequest.update({
      where: { id },
      data: {
        status: "COMPLETED",
        processedAt: new Date(),
...
router.post("/dpdp/consent", authMiddleware, requireTenant, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { memberId, purpose, consentGiven } = z.object({
      memberId: z.string(),
      purpose: z.string(),
...
router.get("/dpdp/consent", authMiddleware, requireTenant, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
