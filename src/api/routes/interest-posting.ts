...
import { postInterestWithTDS } from "../../services/tds-integration.service";
...
router.post("/post/sb", authMiddleware, requireTenant, async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        const tenantId = req.user!.tenantId!;
        const { postingDate, accountIds } = z.object({
            postingDate: z.string().optional().transform((s) => s ? new Date(s) : new Date()),
        ...
        // Group by account
        const accountGroups = new Map<string, typeof accruals>();
        ...
        for (const [accountId, accountAccruals] of accountGroups) {
            try {
                ...
                // Get SB account
                const sbAccount = await prisma.sbAccount.findUnique({
                    where: { id: accountId },
                    include: { member: true },
                });
                ...
                // GL posting
                await postGl(tenantId, "SB_INTEREST_ACCRUAL", totalInterest,
                    `SB interest credit — ${sbAccount.accountNumber}`, period);
        ...
        await createAuditLog(tenantId, "SB_INTEREST_POSTED", {
...
router.post("/post/fdr", authMiddleware, requireTenant, async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        const tenantId = req.user!.tenantId!;
        const { postingDate, depositIds } = z.object({
            postingDate: z.string().optional().transform((s) => s ? new Date(s) : new Date()),
        ...
        // Group by deposit
        const depositGroups = new Map<string, typeof accruals>();
        for (const accrual of accruals) {
        ...
        for (const [depositId, depositAccruals] of depositGroups) {
            try {
                ...
                const tdsResult = await postInterestWithTDS(
                    tenantId,
                    depositId,
                    "FDR",
                    deposit.memberId,
                ...
                if (sbAccount) {
                    ...
                    await postGl(tenantId, "FDR_INTEREST_ACCRUAL", tdsResult.netCreditAmount,
                        `FDR interest credit — ${deposit.depositNumber}`, period);

                    // GL Posting: TDS liability (if applicable)
                    if (tdsResult.tdsApplicable && tdsResult.tdsAmount > 0) {
                        await postGl(tenantId, "FDR_TDS_DEDUCTED", tdsResult.tdsAmount,
                            `TDS deduction — ${deposit.depositNumber}`, period);
        ...
        await createAuditLog(tenantId, "FDR_INTEREST_POSTED", {
            postingDate: postingDateObj.toISOString(),
