...
import { calculateInterest, getActiveScheme } from "./interest-calculation.service";
 ...
 */
async function validateBackdateWindow(
    tenantId: string,
    effectiveFromDate: Date
): Promise<{ valid: boolean; error?: string }> {
    const maxBackdateConfig = await prisma.systemConfig.findFirst({
        where: {
            tenantId: "PLATFORM",
...
export async function recalculateInterest(
    ...
    try {
        ...
        if (accountType === "SB") {
            account = await prisma.sbAccount.findUnique({
                where: { id: accountId },
                include: { member: true },
            });
            if (!account) {
            ...
            account = await prisma.deposit.findUnique({
                where: { id: accountId },
                include: { member: true },
            });
            if (!account) {
        ...
        return await prisma.$transaction(async (tx) => {
            ...
            const totalReversalAmount = accrualsToReverse.reduce(
            ...
            await postGl(tenantId, reversalGlType, -totalReversalAmount,
                `Interest reversal — ${account.accountNumber || account.depositNumber} (Backdated recalculation)`, period);
            ...
            while (recalculationDate <= today) {
                try {
                    ...
                    if (result.interestAmount >= 0.01) {
                        // Get scheme ID
                        const schemeData = await getActiveScheme(tenantId, accountType, recalculationDate);
 ...
 */
export async function isRecalculationInProgress(
    tenantId: string,
    accountId: string,
    accountType: "SB" | "FDR" | "RD"
): Promise<boolean> {
    ...
    const recentRecalc = await prisma.auditLog.findFirst({
        where: {
            tenantId,
            action: "INTEREST_RECALCULATION",
            entity: accountType === "SB" ? "SbAccount" : "Deposit",
