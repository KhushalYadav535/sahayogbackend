...
router.get("/nabard-report", auth_1.authMiddleware, auth_1.requireTenant, async (req, res) => {
    try {
        ...
        const [members, deposits, loans, glSummary] = await Promise.all([
            ...
            prisma_1.default.glEntry.groupBy({
                by: ["glName"],
...
router.get("/tds-26q", auth_1.authMiddleware, auth_1.requireTenant, async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        const { quarter } = req.query; // 2024-Q1 format
...
router.get("/tds-records", auth_1.authMiddleware, auth_1.requireTenant, async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        const { fy } = req.query;
        const financialYear = fy || "2025-26";
        // Get deposits with TDS deducted
        const tdsDeposits = await prisma_1.default.deposit.findMany({
            where: {
                tenantId,
                OR: [
                    { tdsDeducted: { gt: 0 } },
...
router.get("/tds-quarterly", auth_1.authMiddleware, auth_1.requireTenant, async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        const { fy } = req.query;
        const financialYear = fy || "2025-26";
...
router.post("/tds-certificates", auth_1.authMiddleware, auth_1.requireTenant, async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        const { fy } = req.body;
        const financialYear = fy || "2025-26";
        // Get deposits with TDS for certificate generation
        const certificates = await prisma_1.default.deposit.findMany({
            where: {
                tenantId,
                tdsDeducted: { gt: 0 },
                OR: [
...
router.get("/str", auth_1.authMiddleware, auth_1.requireTenant, async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
...
router.get("/aml", auth_1.authMiddleware, auth_1.requireTenant, async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        ...
        const [flaggedTx, kycPending] = await Promise.all([
            ...
            prisma_1.default.member.count({ where: { tenantId, kycStatus: "pending" } }),
        ]);
...
router.get("/registrar-return", auth_1.authMiddleware, auth_1.requireTenant, async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        const { fy } = req.query;
        const financialYear = fy || "2025-26";
        ...
        const shareCapital = await prisma_1.default.shareLedger.aggregate({
...
router.get("/26as-ais", auth_1.authMiddleware, auth_1.requireTenant, async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        const { fy, pan } = req.query;
        const financialYear = fy || "2025-26";
        const deposits = await prisma_1.default.deposit.findMany({
            where: {
                tenantId,
...
router.get("/dashboard", auth_1.authMiddleware, auth_1.requireTenant, async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        const now = new Date();
        ...
        const [nabardDue, registrarDue, tdsQuarterly, strPending, kycPending, amlAlerts, complianceEvents,] = await Promise.all([
            ...
            prisma_1.default.deposit.count({
                where: {
                    tenantId,
                    tdsDeducted: { gt: 0 },
                    updatedAt: {
            ...
            // STR pending count
            prisma_1.default.amlAlert.count({
                where: { tenantId, status: "PENDING", alertType: { in: ["CTR", "STRUCTURING", "UNUSUAL_PATTERN"] } },
            }).catch(() => 0),
            // KYC pending
            prisma_1.default.member.count({ where: { tenantId, kycStatus: "pending" } }),
            // AML alerts
            prisma_1.default.amlAlert.count({ where: { tenantId, status: "PENDING" } }).catch(() => 0),
            // Compliance events
...
router.get("/member-due-report", auth_1.authMiddleware, auth_1.requireTenant, async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
...
router.get("/member-ledger", auth_1.authMiddleware, auth_1.requireTenant, async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
...
router.get("/member-list", auth_1.authMiddleware, auth_1.requireTenant, async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
...
router.get("/audit-support-package", auth_1.authMiddleware, auth_1.requireTenant, async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        const { fy } = req.query;
        const financialYear = fy || "2025-26";
        const [trialBalance, vouchers, members, loans, deposits, transactions, auditLogs,] = await Promise.all([
            prisma_1.default.glEntry.groupBy({
                by: ["glName"],
                where: { tenantId },
                _sum: { debit: true, credit: true },
...
router.get("/slr-crr-report", auth_1.authMiddleware, auth_1.requireTenant, async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
...
router.get("/gst-invoice", auth_1.authMiddleware, auth_1.requireTenant, async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
...
router.get("/loan-schedule-report", auth_1.authMiddleware, auth_1.requireTenant, async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
...
router.get("/deposit-maturity-schedule", auth_1.authMiddleware, auth_1.requireTenant, async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        const { from, to, depositType } = req.query;
...
router.get("/regulatory-notifications", auth_1.authMiddleware, auth_1.requireTenant, async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
...
router.get("/income-tax-exports", auth_1.authMiddleware, auth_1.requireTenant, async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        const { fy, format, memberId } = req.query;
        const financialYear = fy || "2025-26";
