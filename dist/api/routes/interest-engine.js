...
router.get("/schemes", auth_1.authMiddleware, auth_1.requireTenant, async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        const { productType, status } = req.query;
        const schemes = await prisma_1.default.interestScheme.findMany({
            where: {
                tenantId,
...
router.get("/schemes/pending", auth_1.authMiddleware, auth_1.requireTenant, async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        const schemes = await prisma_1.default.interestScheme.findMany({
            where: {
...
router.get("/schemes/audit", auth_1.authMiddleware, auth_1.requireTenant, async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        ...
        const auditRecords = await prisma_1.default.interestSchemeAudit.findMany({
            where,
            include: {
                scheme: {
                    select: {
...
router.get("/schemes/audit/export", auth_1.authMiddleware, auth_1.requireTenant, async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
...
router.get("/schemes/:id", auth_1.authMiddleware, auth_1.requireTenant, async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
...
router.post("/schemes", auth_1.authMiddleware, auth_1.requireTenant, async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        const makerId = req.user.id;
        const data = zod_1.z.object({
            schemeCode: zod_1.z.string().min(1).max(20),
            schemeName: zod_1.z.string().min(1).max(200),
            productType: zod_1.z.enum(["SB", "FDR", "RD", "Loan"]),
            interestMethod: zod_1.z.enum(["SIMPLE", "REDUCING_BALANCE", "FLAT"]),
            compoundingFreq: zod_1.z.enum(["MONTHLY", "QUARTERLY", "ANNUALLY", "SIMPLE"]),
            slabApplicationMethod: zod_1.z.enum(["FLAT", "MARGINAL"]).default("FLAT"),
            effectiveFromDate: zod_1.z.string().transform((s) => new Date(s)),
            effectiveToDate: zod_1.z.string().optional().transform((s) => s ? new Date(s) : undefined),
            slabs: zod_1.z.array(zod_1.z.object({
                minAmount: zod_1.z.number().optional(),
                maxAmount: zod_1.z.number().optional(),
                minTenureDays: zod_1.z.number().optional(),
                maxTenureDays: zod_1.z.number().optional(),
        ...
        const overlapping = await prisma_1.default.interestScheme.findFirst({
            where: {
                tenantId,
        ...
        const scheme = await prisma_1.default.interestScheme.create({
            data: {
                tenantId,
        ...
        if (data.slabs && data.slabs.length > 0) {
            await prisma_1.default.interestSchemeSlab.createMany({
        ...
        await (0, audit_1.createAuditLog)(tenantId, "INTEREST_SCHEME_CREATED", {
            schemeId: scheme.id,
...
router.put("/schemes/:id/submit", auth_1.authMiddleware, auth_1.requireTenant, async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        ...
        let rateDeltaPct = null;
        if (scheme.makerId) {
            // This is a modification - calculate delta
            const oldScheme = await prisma_1.default.interestSchemeAudit.findFirst({
        ...
        await (0, audit_1.createAuditLog)(tenantId, "INTEREST_SCHEME_SUBMITTED", {
            schemeId,
            makerId: req.user.id,
        });
...
router.put("/schemes/:id/approve", auth_1.authMiddleware, auth_1.requireTenant, async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        ...
        if (scheme.status === "PENDING_APPROVAL") {
            await prisma_1.default.interestScheme.updateMany({
                where: {
                    tenantId,
        ...
        await prisma_1.default.interestSchemeAudit.create({
            data: {
                schemeId,
        ...
        await (0, audit_1.createAuditLog)(tenantId, "INTEREST_SCHEME_APPROVED", {
            schemeId,
            checkerId,
        });
        res.json({ success: true, scheme: updated });
...
router.put("/schemes/:id/reject", auth_1.authMiddleware, auth_1.requireTenant, async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        const checkerId = req.user.id;
        const schemeId = req.params.id;
        ...
        await (0, audit_1.createAuditLog)(tenantId, "INTEREST_SCHEME_REJECTED", {
            schemeId,
            checkerId,
            reason: data.reason,
        });
...
router.post("/simulate", auth_1.authMiddleware, auth_1.requireTenant, async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        const data = zod_1.z.object({
            productType: zod_1.z.enum(["SB", "FDR", "RD", "Loan"]),
            principal: zod_1.z.number().min(0),
            rate: zod_1.z.number().min(0).max(100).optional(),
            tenureDays: zod_1.z.number().optional(),
            tenureMonths: zod_1.z.number().optional(),
            compoundingFreq: zod_1.z.enum(["MONTHLY", "QUARTERLY", "ANNUALLY", "SIMPLE"]).optional(),
            memberAge: zod_1.z.number().optional(),
        }).parse(req.body);
...
//# sourceMappingURL=interest-engine.js.map