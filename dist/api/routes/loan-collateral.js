...
router.post("/applications/:id/collateral", auth_1.authMiddleware, auth_1.requireTenant, async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        const userId = req.user.id;
        const data = zod_1.z.object({
            collateralType: zod_1.z.enum(["PROPERTY", "GOLD", "VEHICLE", "FDR", "SHARES", "OTHER"]),
            description: zod_1.z.string().optional(),
            valuationDate: zod_1.z.string().transform((s) => new Date(s)),
            valuationAmount: zod_1.z.number().positive(),
            valuerName: zod_1.z.string().optional(),
            // Gold-specific (LN-COL02)
            goldPurity: zod_1.z.string().optional(),
            goldGrossWeight: zod_1.z.number().positive().optional(),
            goldNetWeight: zod_1.z.number().positive().optional(),
            goldCertRefNumber: zod_1.z.string().optional(),
            goldRatePerGram: zod_1.z.number().positive().optional(),
            // Property-specific (LN-COL03)
            chargeType: zod_1.z.enum(["EQUITABLE_MORTGAGE", "REGISTERED_MORTGAGE"]).optional(),
            registrationDate: zod_1.z.string().transform((s) => new Date(s)).optional(),
            subRegistrarOffice: zod_1.z.string().optional(),
        ...
        const collateral = await prisma_1.default.collateral.upsert({
        ...
        await (0, audit_1.createAuditLog)(tenantId, userId, "LOAN_COLLATERAL_CREATED", {
            applicationId: application.id,
            collateralType: data.collateralType,
            valuationAmount: data.valuationAmount,
...
router.post("/collateral/gold/calculate", auth_1.authMiddleware, auth_1.requireTenant, async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        const data = zod_1.z.object({
            purity: zod_1.z.string(), // e.g., "22K", "24K"
            grossWeight: zod_1.z.number().positive(),
            netWeight: zod_1.z.number().positive(),
            goldRatePerGram: zod_1.z.number().positive(),
        }).parse(req.body);
...
router.get("/collaterals", auth_1.authMiddleware, auth_1.requireTenant, async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        ...
        if (loanId)
            where.loanId = loanId;
        const collaterals = await prisma_1.default.collateral.findMany({
            where,
            include: {
                application: {
                    include: {
...
//# sourceMappingURL=loan-collateral.js.map