...
router.post("/aadhaar/ekyc/initiate", auth_1.authMiddleware, auth_1.requireTenant, async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        const { aadhaarNumber, memberId } = zod_1.z.object({
            aadhaarNumber: zod_1.z.string().length(12).regex(/^\d{12}$/),
        ...
        await prisma_1.default.member.updateMany({
            where: {
                tenantId,
...
router.post("/aadhaar/ekyc/verify", auth_1.authMiddleware, auth_1.requireTenant, async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        const { otpReference, otp, memberId } = zod_1.z.object({
            otpReference: zod_1.z.string(),
...
router.post("/upi/generate-qr", auth_1.authMiddleware, auth_1.requireTenant, async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        const { amount, purpose, transactionId, upiId } = zod_1.z.object({
        ...
        try {
            await prisma_1.default.paymentRequest.create({
                data: {
                    tenantId,
                    orderId,
                    amount,
...
router.post("/upi/webhook", async (req, res) => {
    try {
        ...
        try {
            ...
            await prisma_1.default.paymentRequest.update({
                where: { orderId },
                data: {
...
router.post("/sms/send", auth_1.authMiddleware, auth_1.requireTenant, async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        const { to, templateId, params, memberId } = zod_1.z.object({
        ...
        // Check SMS credits
        const tenantCredits = await prisma_1.default.tenantCredits.findUnique({ where: { tenantId } });
...
router.post("/sms/webhook", async (req, res) => {
    try {
        ...
        try {
            await prisma_1.default.notificationLog.updateMany({
                where: { messageId },
                data: {
...
router.post("/payment-gateway/create-order", auth_1.authMiddleware, auth_1.requireTenant, async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        const { amount, purpose, transactionId, gateway } = zod_1.z.object({
            amount: zod_1.z.number().positive(),
            purpose: zod_1.z.string(),
            transactionId: zod_1.z.string().optional(),
            gateway: zod_1.z.enum(["razorpay", "payu"]).default("razorpay"),
        }).parse(req.body);
...
router.get("/bulk-export/:type", auth_1.authMiddleware, auth_1.requireTenant, async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
...
router.post("/bulk-import/:type", auth_1.authMiddleware, auth_1.requireTenant, async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        const { type } = req.params;
        const { data, validateOnly } = zod_1.z.object({
...
router.post("/nach/register", auth_1.authMiddleware, auth_1.requireTenant, async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        const { memberId, bankAccount, amount, frequency, startDate } = zod_1.z.object({
        ...
        try {
            await prisma_1.default.nachMandate.create({
                data: {
                    tenantId,
                    memberId,
                    mandateId,
